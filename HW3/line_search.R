...
optimal_step <- function(param_act, direct, ll_act, grad_ll_act, y, X, mi, c=0.01, max_alpha=1, rho=0.5){
  opt_alpha <- max_alpha
  while(log_lik(param_act + opt_alpha*direct, y, X, mi) > ll_act + c*opt_alpha*crossprod(grad_ll_act,direct)){
    opt_alpha <- opt_alpha*rho
  }
  return(opt_alpha)
}

# Function for the gradient descent of the logit model
GD_line_search <- function(y, X, mi, beta0, maxiter, tol){
  ...
  for (iter in 2:maxiter){
    ...
    direct <- -gradient
    alpha[iter-1] <- optimal_step(betas[iter-1,], direct, ll[iter-1], gradient, y, X, mi)
    ...
    else if (iter == maxiter & abs(ll[iter-1] - ll[iter])/abs(ll[iter-1] + 1E-2) >= tol){
...
quasi_newton <- function(y, X, mi, beta0, maxiter, tol){
