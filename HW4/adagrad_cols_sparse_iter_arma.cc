...
// [[Rcpp::export]]
vec comp_wi(sp_mat& X, vec beta, unsigned int idx){
	...
	sp_mat::iterator a = X.begin_col(idx); // start of column idx
	sp_mat::iterator b = X.end_col(idx);   // end of column idx

	for(sp_mat::iterator i = a; i != b; ++i){
		xbeta -= (*i) * beta(i.row());
	}

	// Compute the probabilities
	wi = 1.0 / (1.0 + exp(xbeta));
	
	return wi;
}

// [[Rcpp::export]]
double log_lik(vec beta, vec y, sp_mat& X, vec mi, unsigned int idx, double lambda = 0.0){
...
// [[Rcpp::export]]
vec grad_loglik(vec beta, vec y, sp_mat& X, vec mi, unsigned int idx, double lambda = 0.0){
	...
	grad_ll = - X.col(idx) * (y - mi * wi) + 2.0 * lambda * beta;
...
// [[Rcpp::export]]
vec Ada_Grad(vec y, sp_mat& X, vec mi, vec beta0, double eta = 1.0, double lambda = 0.0, unsigned int maxiter = 1000000, double tol = 1E-12){
	...
	//av_ll(0) = log_lik(betas.col(0), y_samp, X, mi_samp, 0, lambda);
	...
	for (unsigned int iter = 0; iter < maxiter; ++iter){
		...
		//grad = grad_loglik(betas.col(iter - 1), y_samp, X, mi_samp, idx, lambda);
		grad = grad_loglik(betas, y_samp, X, mi_samp, idx, lambda);
		...
		//av_ll(iter) = (av_ll(iter - 1) * iter + log_lik(betas.col(iter), y_samp, X, mi_samp, idx, lambda)) / (iter + 1);
