...
source('../SDS385/HW1/gradient_newton.R')
...
# We compute the GLM result in order to compare different algorithms
...
# Gradient descent using, as a diagnostic of convergence, the log-likelihood of the 
# entire dataset
...
# We display the step size as a function of iterations
...
sgd.robbins <- SGD.Robbins(y, X, mi, beta0, maxiter, C, t0, alpha, tol)

par(mar=c(4,2,2,2),mfrow=c(3,4))
for (i in 1:11){
  plot(sgd.robbins$beta[seq(1,maxiter,by=40),i],type='l',ylab='',xlab='Iterations',main=bquote('Values of'~beta[.(i)]))
  abline(h=newton$beta[i],col='red',lwd=2)
}

plot(sgd.robbins$ll, type = 'l', lwd = 2, col = 'red', xlab = 'Iterations', ylab = 'Negative individual Log-likelihood')

# (E)

par(mar=c(4,2,2,2),mfrow=c(3,4))
for (i in 1:11){
  plot(sgd.robbins$beta[seq(maxiter/2,maxiter,by=20),i],type='l',ylab='',xlab='Iterations',main=bquote('Values of'~beta[.(i)]))
  abline(h=newton$beta[i],col='red',lwd=2)
}
...
plot(sgd$ll, type = 'l', lwd = 2, col = 'red', xlab = 'Iterations', ylab = 'Negative individual Log-likelihood')
plot(sgd.robbins$ll, type = 'l', lwd = 2, col = 'red', xlab = 'Iterations', ylab = 'Negative Log-likelihood')
